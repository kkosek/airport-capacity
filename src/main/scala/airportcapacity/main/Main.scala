...
import airportcapacity.service.{
  AirportConfig,
  AirportService,
  FlightInfoConfig,
  FlightInfoService
...
trait Setup {
  ...
  lazy val airportConfig = AirportConfig(
    config.getStringList("airport.ids").asScala.map(AirportId),
    config.getString("airport.file"))
  lazy val flightInfoConfig = FlightInfoConfig(
    config.getString("flight-info.open-sky-url"))
  ...
  lazy val cassandraConfig = DbConfig(
    ...
    defaultConsistencyLevel =
      config.getString("cassandra.default-consistency-level"),
...
object Main extends App with Setup {
  ...
  aggregatorService.source.runWith(aggregatorService.sink)
}
